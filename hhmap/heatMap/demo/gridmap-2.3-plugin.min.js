    ...
    ("undefined" != typeof window.Maplet || "undefined" != typeof MMap) && ("undefined" != typeof window.Maplet ? Maplet : MPlugin).define("gridmap", function() {
        return new function() {
            var a = null,
                ...
                q = null,
                r = null,
                s = { DbName: "Felankia_GridMap", DataObjName: "Datas", CountObjName: "Counts", version: 22, db: null },
                t = [];
            this.init = function(b, e) {
                j = e || j, c = document.createElement("canvas");
                var h = document.createElement("div");
                if (h.style.width = "0px", h.style.height = "0px", h.appendChild(c), q = document.createElement("div"), q.style.display = "none", q.style.position = "absolute", document.body.appendChild(q), a = b, MEvent.addListener(a, "zoom", function() { w(), z() }), MEvent.addListener(a, "pan", z), d = new MMarker(new MPoint(0, 0), new MIcon(h, 0, 0)), d.autoHide = !1, a.addOverlay(d), r = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.mozIndexedDB, !r) return void console.log("您的浏览器不支持IndexedDB！");
                var k = r.open(s.DbName, s.version);
                k.onerror = function() { console.log("IndexedDB open error!") }, k.onsuccess = function(a) {
                ...
                }, k.onupgradeneeded = function(a) { a.target.result.objectStoreNames.contains(s.DataObjName) || a.target.result.createObjectStore(s.DataObjName, { keyPath: "geohash" }), a.target.result.objectStoreNames.contains(s.CountObjName) || a.target.result.createObjectStore(s.CountObjName, { keyPath: "level" }) }, c.onmousemove = function(b) {
                ...
                }, c.onmouseout = function() { q.style.display = "none" }
            }, this.reset = function(a) {
                e = [], f = [], g = [], i = 0;
                var b = s.db.transaction([s.DataObjName, s.CountObjName], "readwrite");
                b.oncomplete = function() { z(), a && a() }, b.onerror = function(a) { console.log("IndexedDB clear error:" + a) }, b.objectStore(s.DataObjName).clear(), b.objectStore(s.CountObjName).clear()
            ...
            var u = function(a, b) {
                    ...
                    c.oncomplete = function() { z(), b && b() }, c.onerror = function(a) { console.log("IndexedDB write error:" + a) };
                    var d = c.objectStore(s.DataObjName);
                    for (var e in a) ! function(b) { d.get(b).onsuccess = function(c) { d.put({ geohash: b, value: a[b] + (c.target.result ? c.target.result.value : 0) }) } }(e);
                    var h = c.objectStore(s.CountObjName);
                    h.clear();
                    for (var j in f) h.put({ level: parseInt(j), count: f[j], max: g[j] });
                    h.put({ level: "all", total: i })
                },
                v = function(a, b) {
                    if (b)
                        if (e[a]) b(e[a]);
                            ...
                            c.onerror = function(a) { console.log("IndexedDB read error:" + a) };
                            var d = c.objectStore(s.DataObjName);
            ...
            this.setSize = function(a) { l = a, m = 256 >> l, z() }, this.setRander = function(a) { p = a ? a : x(), z() }, this.hide = function() { n = !1, z() }, this.show = function(a, b) { n = !0, null != a && this.setSize(a), b && this.setRander(b), p || (p = x()), z() };
            var x = function() {
                    return function(a) {
                        var b = { fillType: "solid", lineWidth: 0, strokeStyle: "rgba(255,255,255,0)" },
                            c = null;
                        return 0 == a.value ? b.fillStyle = "rgba(255,255,255,0)" : (c = { showType: "popup", content: a.value }, b.fillStyle = "rgba(255,0,0," + a.value / a.max + ")"), { rect: b, text: c }
                    }
                },
                y = function(b) { return t.MHeatMap && (pos = a.toScreenCoordinate(b[0] + "," + b[1])) && t.MHeatMap.getDataAt(pos[0], pos[1]) || h[F(B(H(b), k))] },
                z = function() {
                            ...
                            ! function(b, d, e) {
                                b = F(b), v(b, function(l) {
                                    var n = (d - o.left) / x,
                                        ...
                                        u = p({ value: l, total: i, max: g[k], count: f[k] }, { zoom: k, gridSizePixel: m, gridBound: { left: d - o.left, top: e - o.down, rigth: d - o.left + r, bottom: e - o.down + r } });
                                    if (u.rect) {
                                        switch (j.beginPath(), j.rect(n, q, m, m), u.rect.fillType.toLowerCase()) {
                                            ...
                                            case "image":
                                                var B = new Image;
                                                B.src = u.rect.fillStyle, B.complete ? j.drawImage(B, n, q, m, m) : (B.onload = function() { j.drawImage(B, n, q, m, m) }, B.onerror = function() { console.log("图片加载失败:" + B.src) });
                                                break;
                                            case "heatmap":
                                                if (l > 0) {
                                                    var C = I([d + s, e + s]);
                                                    y.push({ lon: C[0], lat: C[1], value: l })
                                                }
                                        }
                                    ...
                                    0 == --z && y.length > 0 && (a.hideHeatmap(), t.MHeatMap = a.enableHeatmap({ minZoomLevel: a.getZoomLevel(), maxZoomLevel: a.getZoomLevel(), radius: 4 * m / 3, dataMax: g[k], dataMin: 0 }, y), a.showHeatmap())
                                })
                            }(E, C, D)
                        }
                ...
                A = function() {
                    var b = a.getViewBound(),
                        ...
                        g = C(B(H([c, f]), k)),
                        h = C(B(H([e, d]), k));
                    o = { left: g[0], up: h[1], right: h[0], down: g[1] }
                },
                B = function(a, c) { for (var d = D(a[0], -b, b, c), e = D(a[1], -b, b, c), f = new Array, g = 0; c > g; g++) f.push(d[g]), f.push(e[g]); return f.join("") },
                C = function(a) {
                ...
                E = function(a, b, c) { for (var d = 0, e = 0; e < a.length; e++) d = (b + c) / 2, "1" == a[e] ? b = d : c = d; return b },
                F = function(a) { for (var b = [], c = 0; c < a.length; c += 4) b.push(c + 3 < a.length ? String.fromCharCode(65 + 8 * a[c] + 4 * a[c + 1] + 2 * a[c + 2] + 1 * a[c + 3]) : 2 * a[c] + 1 * a[c + 1]); return b.join("") },
                G = function(a) {
